...
///  * [Step]
///  * <https://material.io/archive/guidelines/components/steppers.html>
class AxStepper extends StatefulWidget {
  ...
  @override
  State<AxStepper> createState() => _AxStepperState();
}

class _AxStepperState extends State<AxStepper> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(AxStepper oldWidget) {
  ...
  Widget _buildLine(bool visible) {
    return Container(color: Colors.grey.shade400, width: visible ? 1.0 : 0.0, height: 16.0);
  }
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    ...
    switch (state) {
      ...
      case StepState.disabled:
        return Text(
          '${index + 1}',
          style: _kStepStyle.copyWith(color: isActive ? Colors.white : context.h1),
        );
      case StepState.editing:
        return Icon(Icons.edit, size: 18.0, color: isActive ? _kCircleActiveDark : _kCircleActiveLight);
      case StepState.complete:
        return Icon(Icons.check, size: 18.0, color: context.primary);
  ...
  Color _circleColor(int index) {
    return widget.steps[index].isActive
        ? (widget.steps[index].state == StepState.complete ? context.bg2 : context.primary)
        : context.bg2;
    // if (!_isDark()) {
    //   return widget.steps[index].isActive ? context.colors.primary : context.bg2;
    // } else {
    //   return widget.steps[index].isActive
    //       ? (widget.steps[index].state == StepState.complete ? context.bg2 : context.primary)
    //       : context.bg2;
  ...
  Color _circleBorderColor(int index) {
    return widget.steps[index].isActive
        ? (widget.steps[index].state == StepState.complete ? context.h3 : context.primary)
        : context.h3;
    // if (!_isDark()) {
    //   return widget.steps[index].isActive ? context.colors.primary : context.colors.hd3;
    // } else {
    //   return widget.steps[index].isActive
    //       ? (widget.steps[index].state == StepState.complete ? const Color(0xFF383B42) : context.primary)
    //       : const Color(0xFF383B42);
    // }
  }
    ...
    return Container(
      ...
      child: AnimatedContainer(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: _circleBorderColor(index), width: 1),
    ...
    return Container(
      ...
      child: Center(
        child: SizedBox(
          ...
          child: CustomPaint(
            painter: _TrianglePainter(color: _isDark() ? _kErrorDark : _kErrorLight),
            child: Align(
  ...
  Widget _buildHorizontalProgressSteps() {
      ...
      for (int i = 0; i < widget.steps.length; i += 1) ...<Widget>[
        InkResponse(
          onTap: widget.steps[i].state != StepState.disabled
              ...
              : null,
          canRequestFocus: widget.steps[i].state != StepState.disabled,
          child: SizedBox(height: 72.0, child: Center(child: _buildIcon(i))),
        ),
        if (!_isLast(i))
          // Line
          Expanded(child: Container(height: 4.0, color: context.bg2)),
      ],
    ];
  ...
  Widget _buildHorizontalProgressWithoutSteps() {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: AnimatedFractionallySizedBox(
          ...
          widthFactor: widget.currentStep / (widget.steps.length - 1),
          child: Container(
            width: double.infinity,
            decoration: BoxDecoration(
              color: context.bg1,
  ...
  Widget _buildHorizontal() {
    ...
    return Column(
      children: <Widget>[
        if (widget.useProgressIndicatorInsteadOfSteps) _buildHorizontalProgressWithoutSteps(),
        // if (!widget.useProgressIndicatorInsteadOfSteps) _buildHorizontalProgressSteps(),
